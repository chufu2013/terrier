...
BENCHMARK_DEFINE_F(ParalleScanBenchmark, ParallelScanNoPredicate)(benchmark::State &state) {  // NOLINT
uint32_t num_row = 10000000;
LoggersUtil::Initialize();
auto db_main = terrier::DBMain::Builder().SetUseGC(true).SetUseGCThread(true).SetUseCatalog(true).Build();
...
BENCHMARK_REGISTER_F(ParalleScanBenchmark, ParallelScan)
    ...
    ->UseRealTime()
    ->UseManualTime();
BENCHMARK_REGISTER_F(ParalleScanBenchmark, ParallelScanNoPredicate)
->Unit(benchmark::kMillisecond)
->UseRealTime()
->UseManualTime();
}  // namespace terrier
